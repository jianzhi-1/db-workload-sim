...
from Scheduler import Scheduler, QueueKernelScheduler, KSMFScheduler, QueueScheduler, SequentialScheduler, QueueBasedScheduler, RapidFireRandomScheduler, RapidFireKRandomScheduler, KSMFTwistedScheduler
...
n_queues = 1
max_n_kernel = 50
filename = "arena_baseline_twisted.txt"
for t in range(T):
    num_txns = 1000
...
contestants.extend(
    [
        #Contestant("int-opt-k=1", QueueKernelScheduler(n_queues=n_queues, kernel=KernelWrapper(IntegerOptimisationKernelMkII(max_n_kernel)))),
        Contestant("k-smf", scheduler = KSMFScheduler(k=50)),
        #Contestant("rapidfire-k-random", scheduler = RapidFireKRandomScheduler(k=50)),
        Contestant("twisted-k-smf", scheduler = KSMFTwistedScheduler(k=50))
    ]
)
